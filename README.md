...
4. Добавим новое поле discount_price и сделаем ему необязательное заполнение, и в шаблоне сделаем условие - если скидка есть, то основная цена зачеркнута и показана цена со скидкой, если скидки нет,то показана основная цена. Также добавим новое поле description в модель для описания и выведем в шаблон.
----------------------------------------------------------------
ЧАСТЬ 2 - Добавление Обьекта в корзину.
-----------------------------------------------------------------
1. Добавим в модель OrderItem новое поле quantity для указания количества обьектов. Миграции.
2. Создадим новую функции во views -> add_to_cart, комментарии по логике будут расставлены в коде. Также импортируем необходимые библиотеки. Пропишем новый путь в url add_to_cart/<slug>/ и добавим в модель Item новый метод get_add_to_cart_url. Это будет похожий на get_absolute_url метод, но в данном случае он будет взаимодействовать с добавлением в корзину. Привязываем данный метод в виде url при нажатии на корзину будет выполнение. Проверяем в админке поля Order = должен появиться заказчик и что он заказал, а в OrderItem должны появиться заказанные предметы и их количество, если всё успешно,то двигаемся далее.
3. Добавим в модель OrderItem два новых поля - user, ordered. Проведем манипуляции со view - order_item, created = OrderItem.objects.get_or_create, в данной строчке будет заключена логика, при котором будет добавляться количество одного обьекта и не будет дубликатов обьектов.
4. Создадим новую функции во views -> remove_from_cart, комментарии прилагаются. Пропишем новый путь в url remove_from_cart/<slug>/  и добавим в модель Item новый метод get_remove_from_cart_url. Пропишем в шаблоне новую опцию для удаления.
5. Добавим сообщения при нажатии на кнопки джанговскими встроенными возможностями. from django.contrib import messages во views. и добавим к нашим двум новым методам удаления и добавления сообщения перед return. Затем в шаблоне _base.html перед <body> прописываем данный тэг и его стили. Теперь при нажатию на кнопку в соответствии с заданной логикой будет выводиться нужное сообщение. Сделаем еще один вывод сообщения по ранее созданному приложению contact в котором у нас присутствует форма для обратной связи. Для этого перейдем в contact/views и для классов в представлении для отображения сообщений необходимо импортировать миксины from django.contrib.messages.views import SuccessMessageMixin, и пропишем сообщение в самом классе. Теперь при отправке сообщения пользователем, будет показан успешный результат. Подробнее о сообщениях - https://docs.djangoproject.com/en/4.0/ref/contrib/messages/
6. На данном этапе у нас создана логика добавления и удалению обьектов в корзине и показ сообщений при успешных операциях. 
----------------------------------------------------------------
ЧАСТЬ 3 - Изменения в UI. Показ данных в корзине.
-----------------------------------------------------------------
1. Теперь давайте сделаем показ корзины только авторизованным юзерам _base.html, в коде комменты. Также добавим пагинацию для списка товаров. Во views добавим к ItemList - paginate_by = 2 и в шаблоне home-page.html перед окончанием блока контент сделаем вывод пагинации с логикой.
2. Затем создадим новый templatetags в shop/templatetags -> создадим функцию cart_item_count которая будет принимать авторизованного юзера и выводить количество его обьектов в корзине. Теперь в шаблоне _base.html в навигации в указании количества обьектов в корзине пропишем {{ request.user|cart_item_count }} в котором будет создан реквест на авторизованного юзера и проверка наличия у него обьектов в корзине, готово.
3. Теперь создадим страницу просмотра заказов для пользователя. Создадим новый класс во view OrderSummaryView, комментарии прилагаются, пропишем в url и создадим такой шаблон order-summary.html. Таблички в шаблоне для отображения можно найти здесь https://mdbootstrap.com/docs/standard/data/tables/ . Далее работаем с шаблоном,комментарии прилагаются. Мы создаем таблицу в которой будут отображены данные заказа - цена,название,стоимость,количество. Также добавим в модель OrderItem три новых метода -> get_total_item_price - умножаем количество на сумму, get_total_discount_item_price - тоже самое но умножение на скидочную цену с проверкой если она имеется. get_amount_saved - вывод разницы между обычной ценой и со скидкой. Данные методы также отображаем в шаблоне, немного анимации. Комментарии прилагаются.
4. Далее в OrderItem добавим метод get_final_price, подсчет общей суммы заказа с проверкой на скидочные цены или нет. А в модели Order добавим get_total который будет вести подсчет суммы заказа уже с нужными ценами. Выводим в шаблон, комментарии прилагаются.
5. далее добавим возможность изменять количество в шаблоне. Добавим новый класс во view remove_single_item_from_cart который будет уменьшать количество на -1 если количество больше одного и удалять если количество было равным одному, . Пропишем данный класс в url. добавим в шаблон кнопки + и - , добавим к ним url, которые будут прибавлять и убавлять количество. Тестим. Данная часть довольно большая в плане кода, но в репозитории все работает, копипастим по необходимости.
----------------------------------------------------------------
ЧАСТЬ 4 - Работа с формой обработки заказа. Checkout 
-----------------------------------------------------------------
1. Для начала создадим новый шаблон checkout, который будет содержать в себе форму оформления заказа. Затем создадим новый forms.py и в нем пропишем форму, комментарии прилагаются. Также установим библиотеку pipenv install django-countries для выбора страны в форме и зарегистрируем в settings 'django_countries'. Создадим новую модель Adress, а во views новый класс CheckoutView. С данного момента становится код более запутанным.так что если появились сложности есть смысл закопипастить, проверено всё работает. Добавим в url checkout и создадим такой шаблон
2. После этого создадим во view еще один класс PaymentView. Данный класс уже будет отображать логику после заполнения формы адреса и метода оплаты и будет отвечать за оплату. Добавим в url payment и создадим такой шаблон. Будем использовать stripe-js. https://stripe.com/docs/payments/elements. Pipenv install stripe. Создадим новую модель Payment и добавим поле в Order -> payment.
3. Кароч дальше идет жесть ... копируем. Купоны тоже. https://github.com/madprogrammer/django-sberbank апишка для подключения сбера. https://dashboard.stripe.com/test/dashboard - все таки если страйп то регимся как компания и берем ключи. Настоящая оплата будет возможно только после доп. регистрации.
4. Функционал возврата - 
